        ...
        while (len > 0)
        {
          ...
          else if ((nsent < 0) && (errno != EINTR))
          {
            if ((errno == EWOULDBLOCK) || (errno == EAGAIN))
            {
              ...
              LOG_ERROR("SAS connection to %s:%s locked up: %d %s", _sas_address.c_str(), SAS_PORT, errno, ::strerror(errno));
            }
            else
            {
              // The socket has failed.
              LOG_ERROR("SAS connection to %s:%s failed: %d %s", _sas_address.c_str(), SAS_PORT, errno, ::strerror(errno));
            }
            ::close(_sock);
            _sock = -1;
            break;
...
bool SAS::Connection::connect_init()
{
  ...
  if (rc != 0)  
  {
    LOG_ERROR("Failed to get addresses for SAS %s:%s : %d %s",
  ...
  for (p = addrs; p != NULL; p = p->ai_next) 
  {
    ...
    if (rc < 0)
    {
      LOG_ERROR("Failed to set send timeout on SAS connection : %d %d %s",
  ...
  if (rc != 0)
  {
    LOG_ERROR("Failed to connect to SAS %s:%s : %d %s", _sas_address.c_str(), SAS_PORT, errno, ::strerror(errno));
    return false;
  }

  freeaddrinfo(addrs);  

  LOG_DEBUG("Connected SAS socket to %s:%s", _sas_address.c_str(), SAS_PORT);
  ...
  LOG_STATUS("Connected to SAS %s:%s", _sas_address.c_str(), SAS_PORT);
